...
CARLSIM_LIBS  += -L$(CARLSIM_LIB_DIR)/lib -lCARLsim `pkg-config --libs opencv`
...
$(local_prog): $(local_src) $(local_objs)
	$(NVCC) -g $(CARLSIM_INCLUDES) $(CARLSIM_FLAGS) $(local_objs) $< -o $@ $(CARLSIM_LFLAGS) $(CARLSIM_LIBS)
